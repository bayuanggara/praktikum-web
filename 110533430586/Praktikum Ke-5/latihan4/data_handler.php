<?php
	...
	function data_handler($root){
		...
		//query untuk menyeleksi seluruh isi tabel MHS
			...
			if(isset($_GET['act']) && $_GET['act'] != ''){
				switch($_GET['act']){
					...
					?>
								...
								//query untuk penghapusan data
								$sql = 'DELETE FROM mahasiswa WHERE nim='.$id;
								$res = mysql_query($sql);
									//jika data dihapus
									if($res){ ?> 
									<script type="text/javascript">
											document.location.href="./"; //redireksi halaman
									</script>
									<?php
									}else{ //jika data tidak terhapus
	...
	function show_admin_data($root){ ?>
		...
		<?php
		//query untuk menampilkan nim, nama, dan alamat dari tabel MHS
		$sql = 'SELECT nim, nama, alamat FROM ' .MHS;
		$res = mysql_query($sql);
		//jika pernyataan dieksekusi
		if($res){
			...
			if($num){?>
				<div class="tabel">
					...
					<table border="1" width="700" cellpadding="4" cellspacing="0">
						...
						<?php
						$i=1;
						//pengisian tabel dengan indeks berdasarkan jml record
	 ...
	 */
	 ...
	 //query unyuk mrnampilkan nim, nama, dan alamat berdasarkan nim tertentu
	...
	function data_editor($root, $id = 0){
		...
		if(isset($_POST['nim']) && $_POST['nim']){
			...
			if(!$id){
				...
				if($res){?>
				<script type="text/javascript">
					document.location.href="<?php echo $root; ?>";	//redireksi halaman
				</script>
				<?php
				}else{
